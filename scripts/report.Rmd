...
# - status_exists,
# - log
load_status <- function(projects, status_file, log_file) {
  df <- mutate(
    projects,
  ...
  map2_dfr(
    ...
    ~mutate(read_csv(.y, col_types = "ii"), project_id=.x)
  ) %>%
    right_join(
      select(
        df, 
...
```{r}
# delete failed
# file_delete(filter(compile_status, exit_code > 0), by="project_id")$status_file)
```
...
```{r}
# delete failed
# file_delete(filter(semanticdb_status, exit_code > 0), by="project_id")$status_file)
```
...
```{r}
merged_sdbs <- 
  select(semanticdb_status, project_id, origin, project_path, compile_status) %>%
  mutate(
    stats_file=path(project_path, ANALYSIS_DIR, "semanticdbs-4.1.0.csv"),
    stats_exists=file_exists(stats_file),
    bin_file=path(project_path, ANALYSIS_DIR, "semanticdbs-4.1.0.bin"),
...
merged_sdbs_stats <- local({
  ...
  map2_dfr(
    existing$project_id, 
    existing$stats_file, 
    ~mutate(read_csv(.y, col_types = "iii", skip = 1), project_id=.x)
  )
})
