...
stage3_corpus <- read_csv(CORPUS_STAGE3, col_types=cols(
  ...
  metadata_scala_code_test_managed = col_integer(),
  metadata_scala_code_test = col_integer(),
  metadata_scala_code_compile_managed = col_integer(),
  metadata_scala_code_compile = col_integer(),
...
The `r CORPUS_STAGE3` contains only the projects that were selected in stage 1.
...
ggsave(get_corpus_path("corpus-overview.pdf"), width=12)
```
...
exceptions <- read_csv(GLOBAL_IMPLICITS_EXCEPTIONS, col_types="cccccccc")
```
...
```{r}
write_latex_tags(get_corpus_path("stage3-analysis.tex"), prefix="")
```
