...
TOP_PROJECTS_TEX <- get_corpus_path("top-projects.tex")
```
...
```{r}
my_scale <- function (...) {
  log_breaks_fun <- scales::log_breaks(base = 10)
  breaks <- function(x) {
    c(-1, 0, log_breaks_fun(x[x >= 1]))
  }
  
  trans <- 
    scales::trans_new(
      "my-scale", 
      transform=function(x) ifelse(x >= 1, log10(x), x), 
      inverse=function(x) ifelse(x >= 1, 10^x, x), 
      breaks=breaks
    )
...
```{r implicit overview plot, }
local({
  breaks <- c(0:as.integer(cs_end), cs_end)
  labels <- c("0", fmt(10^breaks[-1]))
...
#  scale_y_asinh(sec.axis=sec_axis(trans=my_scale2, name = "A")) +
#  my_scale() +
  geom_segment(aes(xend=project_id, y=cs_start, yend=cs_end), color="#ED6559", alpha=.5) +
  geom_bar(width=1, stat="identity", color="#4C5D7C", alpha=.5) +
#  scale_fill_manual(values=c("cs"="#ED6559", "csi"="#ED6559", "ds"="#4C5D7C", "dsi"="#4C5D7C")) +
  theme(
  ...
  scale_y_continuous(
    ...
    labels=labels,
    sec.axis=sec_axis(
      trans=function(x) x/(-cs_height)+cs_end/cs_height, 
...
corpus_implicitness_overview <- 
  ...
  mutate(
    project_id=forcats::fct_reorder(project_id, ds)
  ) #%>% gather("key", "value", ds, cs)
```

```{r implicit overview plot}
my_scale2 <- function(x) {
  x/(-cs_height)+cs_end/cs_height
}
...
corpus_implicitness_overview %>%
  ...
  scale_y_asinh(sec.axis=sec_axis(trans=my_scale2, name = "A")) +
#  my_scale() +
  geom_segment(aes(xend=project_id, y=cs_start, yend=cs_end), color="#ED6559") +
