...
The ``r CORPUS_STAGE3`` file contains the following columns:
...
projects_using_implicits <- nrow(filter(corpus, implicit_callsites > 0))
projects_defining_implicits <- nrow(filter(corpus, implicit_local_declarations> 0))
...
overview_table(
  ...
  r("projects using implicits pct", percent(projects_using_implicits/nrow(corpus))),
  r("projects defining implicits pct", percent(projects_defining_implicits/nrow(corpus)))
)
```
...
```{r}
phase_info <- function(phase, df, code_column=metadata_scala_code, size_column=size_repo) {
  code_column <- enquo(code_column)
  size_column <- enquo(size_column)
  ...
  df %>%
    summarise(
      ...
      `Project size`=size(sum(!!size_column, na.rm=T))
    ) %>%
    mutate_all(fmt)
}

overview_table(
  phase_info("downloaded projects",   stage1_corpus, code_column=scala_code),
  phase_info("compatible projects",   filter(stage1_corpus, compatible), code_column=scala_code),
  phase_info("removed duplicates",    anti_join(filter(stage1_corpus, compatible), stage2_corpus, by="project_id"), code_column=scala_code),
  phase_info("all projects",          stage2_corpus),
  phase_info("metadata extraction",   filter(stage2_corpus, metadata_exit_code==0), size_column=size),
  phase_info("semanticdb generation", filter(stage2_corpus, semanticdb_exit_code==0), size_column=size),
  phase_info("implicits extraction",  filter(stage2_corpus, implicits_exit_code==0), size_column=size)
)
```
...
```{r}
overview_table(
  r("failed projects - all",                  filter(stage2_corpus, implicits_exit_code != 0)),
