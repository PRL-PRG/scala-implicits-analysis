...
For example, the ghtorrent marks the project as Scala project, but in the end it does not contain any Scala code.

```{r loading data}
corpus <- read_corpus_data(CORPUS_STAGE1_F) %>% filter(scala_code > 0)
```
...
```{r}
overview_table(
  overview_projects("all", corpus),
  overview_projects("sbt", sbt_projects),
  overview_projects("compatible sbt", projects)
)
```
...
```{r}
overview_table(
  r("maven projects",       filter(corpus, build_system=="maven")),
  r("graddle projects",     filter(corpus, build_system=="graddle")),
  r("unknown build projects", filter(corpus, is.na(build_system)))
)
```

### The _unknown_ build projects?
...
filter(corpus, is.na(build_system)) %>% 
  ...
  mutate(project_id=make_link(origin, project_id), size=fmt(size(size))) %>%
...
missing_sbt_version <- 
  ...
  mutate(
    sbt_properties=path(corpus_dir, "all-projects", project_id, "project", "build.properties"), 
...
For the Stage 2 we use the following criteria:
...
- at least `r s2_min_commit_count` commits,
- at least `r s2_min_lifespan_months` months between project creation at github and the last push,
- be in scaladex or have more than `r s2_min_gh_stars1` or less than `r s2_max_duplication1` duplication,
- be in scaladex or have more than `r s2_min_gh_stars2` or less than `r s2_max_duplication2` duplication

```{r}
human_corpus <- function(corpus, view=FALSE, code_column=scala_code) {
...
```{r}
stage2_projects <- filter_stage_2_projects(projects)
```
...
old_stage2_projects <- 
  filter(projects, 
         commit_count >= 2,
         scaladex | (gh_pushed_at-gh_created_at > months(2)),
         dejavu_duplication < .75 | gh_stars >= 5
  )

overview_table(
  overview_projects("old", old_stage2_projects),
  overview_projects("new", stage2_projects)
)

anti_join(stage2_projects,old_stage2_projects, by="project_id") %>%
  transmute(origin, commit_count, scala_code, dejavu_duplication=fmt(dejavu_duplication, digit=2), active=gh_pushed_at-gh_created_at, gh_stars, scaladex) %>% 
  View()
```
...
```{r stage2 overview}
overview_table(
  overview_projects("stage two", stage2_projects),
  r("stage two duplication mean", percent(mean(stage2_projects$dejavu_duplication)))
)
```
...
spark_in_corpus <- filter_spark(corpus)

overview_table(
  overview_projects("spark", spark_in_corpus),
  r("ratio of spark code in corpus", percent(sum(spark_in_corpus$scala_code)/sum(corpus$scala_code, na.rm=T)))
)
```
...
spark_in_stage2 <- filter_spark(stage2_projects)

overview_table(
  overview_projects("stage two spark", spark_in_stage2),
  r("stage two ratio of spark code in corpus", percent(sum(spark_in_stage2$scala_code)/sum(corpus$scala_code, na.rm=T)))
)
```
