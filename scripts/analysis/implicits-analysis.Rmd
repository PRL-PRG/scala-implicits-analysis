...
```{r}
ggsave(OUT_IMPLICIT_CALLSITES_RATIO_PDF, width=9.56, height=3.12, units="in")
ggsave(OUT_IMPLICIT_CALLSITES_RATIO_PNG, width=9.56, height=3.12, units="in", dpi=400)
```
...
```{r}
ggsave(OUT_CALLSITES_LOCATION_RATIO_PDF)
ggsave(OUT_CALLSITES_LOCATION_RATIO_PNG, dpi=400)
```
...
```{r}
ggsave(path(output_dir, "hist-impl-parameters.pdf"), width=4.49, height=7.85, unit="in")
```
...
```{r}
ggsave(path(output_dir, "hist-nested-arguments.pdf"), width=4.61, height=7.85, unit="in")
```
