...
```{r patterns together}
# this should correspond to organization of Section 2
...
patterns_overview_declarations_table <- 
  patterns_overview_table %>%
  transmute(
    `Pattern`=patterns_human_name(pattern),
...
patterns_overview_callsites_table <- 
  patterns_overview_table %>%
  transmute(
    `Pattern`=patterns_human_name(pattern),
...
save_pattern_table <- function(df, fname) {
  df %>%
...
save_pattern_table(patterns_overview_declarations_table, OUT_PATTERNS_DECLARATIONS_TEX)
save_pattern_table(patterns_overview_callsites_table,    OUT_PATTERNS_CALLSITES_TEX)
```

```{r}
patterns_overview_all <- bind_rows(
  patterns_overview_full,
  patterns_overview_all_row
)

pattern_info <- function(pattern, category) {
  ctg <- match.arg(category, patterns_overview_all$cat)
  ptn <- match.arg(pattern, patterns_overview_all$pattern)

  filter(patterns_overview_all, cat==ctg, pattern==ptn) %>%
    mutate_at(vars(ends_with("_pct")), percent) %>%
    as.list()
}
```
...
conversion_ds_from_scala <- filter(pattern_implicit_conversions_ds, def_group_id=="org.scala-lang", def_artifact_id=="scala-library") # cos of the way we define from_scala
...
conversion_cs_from_testlib <- 
  map_dfr(TEST_LIBS, ~filter(pattern_implicit_conversions_cs, str_detect(def_library, .)))
```
...
In terms of declarations, \TgIcAllProjectsDeclaringProjects (43\%) declare at least one
implicit conversion.  Together they define 38,609 conversions with a median
of 4 conversions per project and a standard deviation of 41.  Testing
frameworks have the largest numbers of conversions with \c{scalatest} (868)
and \c{specs2} (684). The rest are mostly libraries.???evidence????
\Scala itself define 608 conversions.????is that the compiler???? Most of
the other conversions are coming from the popular DSLs and the two main
functional-programming libraries (\c{cats} and \c{scalaz}). The only
exception is \c{lichess}\footnote{\Cf an open source chess server
  \url{https://github.com/ornicar/lila/})} which the largest (177K LOC) and
most popular application in the corpus (5K+ stars). It defines over 600
conversions. However, on close inspection, we see that it defines a small
DSL for database management.?????
