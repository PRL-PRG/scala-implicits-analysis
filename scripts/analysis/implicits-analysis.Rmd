...
OUT_IMPLICIT_CALLSITES_RATIO_PNG <- path(output_dir, "implicit-callsites-ratio.png")
OUT_CALLSITES_LOCATION_RATIO_PDF <- path(output_dir, "callsites-location-ratio.pdf")
OUT_CALLSITES_LOCATION_RATIO_PNG <- path(output_dir, "callsites-location-ratio.png")
```
...
declarations_all <- 
  ...
  mutate(
    ...
    def_num_implicit_parameters=replace_na(num_implicit_parameters, 0),
...
ggsave(OUT_IMPLICITS_OVERVIEW_PDF, device=cairo_pdf)
```
...
overview_table(
  ...
  r("projects using implicits rnd pct",    ratio(projects_using_implicits, corpus_noncat), digits=0),
  r("projects defining implicits rnd pct", ratio(projects_defining_implicits, corpus_noncat), digits=0),
  r("implicit call sites rnd pct", percent(n_implicit_callsites/n_callsites), digits=0),
)
```
...
```{r}
filter(pattern_bi_directional_cs, str_detect(declaration_id, "collection"), def_group_id=="org.scala-lang") 
```

```{r}
bic_collections <- filter(pattern_bi_directional_cs, str_detect(declaration_id, "collection"), def_group_id=="org.scala-lang") 
nrow(count(bic_collections, project_id))/nrow(corpus_noncat)
```
...
```{r}
ctx_types_local <- semi_join(parameters_all_context, declarations_local, by=c("mid", "declaration_id"))
ctx_types_unique <- distinct(ctx_types_local, def_library, resolved_type_id)

ctx_top_types <- count(ctx_types_local, type_library, resolved_type_id)  %>% arrange(desc(n))
ctx_top_libs <- count(ctx_types_local, type_library) %>% arrange(desc(n))
ctx_java <- filter(ctx_top_types, type_library=="JDK:JDK:jvm") 
ctx_top_types <- group_by(ctx_top_types, resolved_type_id) %>% summarise(n=sum(n), r=n/nrow(ctx_types_local))
```

```{r}
ctx_top_types %>% arrange(desc(n))%>% print(n=20)
```
...
```{r}
ctx_top_libs  %>% print(n=20)
```

```{r}
prettifier_ds <- filter(parameters_all, resolved_type_id=="org/scalactic/Prettifier#")
prettifier_cs <- callsites_all %>% semi_join(prettifier_ds, by=c("mid", "declaration_id"))
```

```{r}
overview_table(
  r("CTX types", nrow(distinct(ctx_types_local, mid, resolved_type_id))),
  r("CTX java use", sum(ctx_java$n)),
  r("CTX java types", ctx_java),
  r("CTX execution context", percent(filter(ctx_top_types, resolved_type_id=="scala/concurrent/ExecutionContext#")$r)),
  r("CTX prettifier", prettifier_cs),
  r("CTX pretrifier projects", count(prettifier_cs, project_id)),
)
```
...
callsites_ratio <-
  select(corpus_cat, project_id, cat, implicit_callsites, callsites) %>%
    mutate(
      r=if_else(callsites > 0, implicit_callsites/callsites, 0),
      cat=cat_to_fct(cat)
    )

callsites_ratio_median <- median(callsites_ratio$r)
```
...
#  geom_flat_violin(width=.5, alpha=.7, lwd=.2) +
  ...
  geom_boxplot(width=.5, alpha=.7, lwd=.3, outlier.size = 0, outlier.stroke=0, color="black") +
  geom_hline(yintercept=callsites_ratio_median, linetype="dashed", color="black", size=0.3) +
  scale_fill_wsj(labels=fmt_categories) +
  scale_color_wsj(labels=fmt_categories) +
  scale_x_discrete(labels=fmt_categories_short) +
  scale_y_continuous(labels=scales::percent, limits=c(0, .75)) +#, sec.axis = sec_axis(trans=~., labels=scales::percent)) +
...
```{r eval=F}
ggsave(OUT_IMPLICIT_CALLSITES_RATIO_PDF, device=cairo_pdf)
```
...
```{r}
callsites_ratio %>%
  ggplot(aes(x=cat, y=r, fill=cat)) +
  geom_boxplot(alpha=.5, outlier.shape=NA, notch=TRUE, show.legend=FALSE) +
  geom_jitter(alpha=.1, position=position_jitterdodge(jitter.width=1), aes(color=cat), show.legend=FALSE) +
  geom_hline(yintercept=callsites_ratio_median, linetype="dashed", color="black", size=0.5) +
  scale_fill_wsj(labels=fmt_categories) +
  scale_color_wsj(labels=fmt_categories) +
  scale_x_discrete(labels=fmt_categories_short) +
...
```{r}
ggsave(OUT_IMPLICIT_CALLSITES_RATIO_PDF, width=9.56, height=3.12, units="in", device=cairo_pdf)
ggsave(OUT_IMPLICIT_CALLSITES_RATIO_PNG, width=9.56, height=3.12, units="in", dpi=400, device=cairo_pdf)
```

```{r}
overview_table(
  r("implicits callsites pct median", percent(median(callsites_ratio_median))),
  r("implicits test callsites pct median", percent(median(filter(callsites_ratio, cat=="test")$r))),
  r("implicits main callsites pct median", percent(median(filter(callsites_ratio, cat!="test")$r)))
)
```
...
defs_overview <- 
  ...
  mutate(
    callsites_pct=ifelse(implicit_callsites > 0, callsites/implicit_callsites, 0)
  ) %>%
...
#  geom_flat_violin(width=.5, alpha=.7, lwd=.2, trim = T, scale="width", position = position_dodge(.8), draw_quantiles=1) +
  ...
  geom_boxplot(aes(fill=cat), width=.5, alpha=.7, lwd=.2, outlier.size=0, outlier.stroke=0, position=position_dodge(.8), color="black", notch = TRUE) +
...
```{r eval=F}
ggsave(OUT_CALLSITES_LOCATION_RATIO_PDF, device=cairo_pdf)
```
...
defs_overview %>%
  mutate(cat=cat_to_fct(cat)) %>%
  ggplot(aes(x=pattern, y=callsites_pct, fill=cat)) +
  geom_boxplot(alpha=.5, outlier.shape=NA, notch=TRUE) +
  geom_jitter(alpha=.1, position=position_jitterdodge(jitter.width=0.3), aes(color=cat), show.legend=FALSE) +
  scale_fill_wsj(labels=fmt_categories_short) +
  scale_color_wsj() +
  scale_x_discrete(labels=c("external"="External dependencies", "local"="Project local", "scala"="Scala standard library")) +
  scale_y_continuous(labels=scales::percent) +
  ...
  labs(x="", y="Ratio of implicit call sites", fill="Category")
```

```{r}
ggsave(OUT_CALLSITES_LOCATION_RATIO_PDF, device=cairo_pdf)
ggsave(OUT_CALLSITES_LOCATION_RATIO_PNG, dpi=400, device=cairo_pdf)
```
...
```{r}
cs_related_to_IP <- filter(callsites_all, def_kind=="DEF", num_implicit_arguments > 0 | (def_is_implicit & !is.na(parent_id)))

overview_table(
  r("IP in call sites pct", ratio(cs_related_to_IP, callsites_all)),
...
match_declaration_id <- function(regex) {
  filter(declarations_all, str_detect(declaration_id, regex))
}
...
projects_using_declaration <- function(name, id) {
  projects_using(name, match_declaration_id(id))
}
...
overview_table(
  ...
  overview_pattern2(
    projects_using_declaration("IP scala predef", "^scala/Predef\\."),
     c("projects_using_pct", "callsites_pct")
  ),
)
```
...
popular_libraries <- 
  ...
  count(def_library) %>%
  filter(
    !startsWith(def_library, "org.scala-lang:scala-library"),
    !startsWith(def_library, "org.scalatest"),
    !startsWith(def_library, "org.scalactic"),
    !startsWith(def_library, "org.specs2"),
    !startsWith(def_library, "org.scalacheck"),
  ) %>%
  mutate(
    r=n/nrow(callsites_all)
  ) %>%
  top_n(20, r) %>%
  arrange(desc(r))
  
overview_table(
  r("Callsites form slick", percent(filter(popular_libraries, startsWith(def_library, "com.typesafe.slick")) %>% summarise(n=sum(n)/nrow(callsites_all)) %>% .$n)),
  r("Callsites form play", percent(filter(popular_libraries, startsWith(def_library, "com.typesafe.play")) %>% summarise(n=sum(n)/nrow(callsites_all)) %>% .$n)),
  r("Callsites form akka", percent(filter(popular_libraries, startsWith(def_library, "com.typesafe.akka")) %>% summarise(n=sum(n)/nrow(callsites_all)) %>% .$n)),
  r("Use scalactic source position", filter(callsites_all, declaration_id=="org/scalactic/source/Position.apply()."))
)
```

```{r}
overview_table(
  r("IP callsites ", pattern_implicit_parameters_cs),
  r("IP callsites pct", ratio(pattern_implicit_parameters_cs, callsites_all))
)
```
...
### Late trait (Section 5.3.1)

```{r}
JAVA_DATE <- c("^java/time", "^java/util/Date", "^org/joda/time")
JAVA_IO <- c("^java/[n]?io", "^io/netty")

is_from_to <- function(df, id) {
  filter(df, str_detect_multiple(from, id) | str_detect_multiple(to, id))
}
```
...
```{r}
LT_ds_per_project <- count(pattern_late_trait_ds, project_id)
LT_cs_per_project <- count(pattern_late_trait_cs, project_id)
LT_declaration    <- count(pattern_late_trait_cs, def_library, declaration_id)
save_top_n_projects(LT_ds_per_project, LT_cs_per_project, path(output_dir, "LT-top-projects.tex"))
```

```{r}
LT_conditional <- filter(pattern_late_trait_ds, def_num_implicit_parameters > 0)
LT_from_to_java <- filter(pattern_late_trait_ds, from_language=="JAVA" | to_language=="JAVA")
LT_from_primitive <- is_from_to(pattern_late_trait_ds, SCALA_PRIMITIVES)
LT_good <- filter(pattern_late_trait_ds, def_group_id==from_group_id | def_group_id==to_group_id)

overview_table(
  r("LT conditional", LT_conditional),
  r("LT conditional pct", ratio(LT_conditional, pattern_late_trait_ds)),
  r("LT good", LT_good),
  r("LT good pct", ratio(LT_good, pattern_late_trait_ds)),
  r("LT java", LT_from_to_java),
  r("LT java pct", ratio(LT_from_to_java, pattern_late_trait_ds)),
  r("LT java io",      is_from_to(LT_from_to_java, JAVA_IO)),
  r("LT java date",    is_from_to(LT_from_to_java, JAVA_DATE)),
  r("LT java lang",    is_from_to(LT_from_to_java, c("^java/lang/"))),
  r("LT java android", is_from_to(LT_from_to_java, c("^android/"))),
  r("LT java libraries", count(gather(LT_from_to_java, "key", "value", from_group_id, to_group_id), value)),
  r("LT from primitive", LT_from_primitive),
  r("LT from primitive to io", filter(LT_from_primitive, str_detect_multiple(to, JAVA_IO))),
  r("LT from primitive to date", filter(LT_from_primitive, str_detect_multiple(to, JAVA_DATE))),
  r("LT from String", filter(count(LT_from_primitive, from), from == "java/lang/String#")$n),
  r("LT from Int", filter(count(LT_from_primitive, from), from == "scala/Int#")$n),
)
```


### Extension methods (Section 5.3.2)

```{r}
EM_ds_per_project <- count(pattern_extension_methods_ds, project_id)
EM_cs_per_project <- count(pattern_extension_methods_cs, project_id)
EM_declaration    <- count(pattern_extension_methods_cs, def_library, declaration_id)
save_top_n_projects(EM_ds_per_project, EM_cs_per_project, path(output_dir, "EM-top-projects.tex"))
```

```{r}
EM_class_over_def <- filter(pattern_extension_methods_ds, def_is_companion)
EM_implicit_classes <- filter(pattern_extension_methods_ds, def_is_companion)
EM_conditional <- filter(pattern_extension_methods_ds, def_num_implicit_parameters > 0)
EM_from_to_java <- filter(pattern_extension_methods_ds, from_language=="JAVA" | to_language=="JAVA")

EM_from_primitive <- is_from_to(pattern_extension_methods_ds, SCALA_PRIMITIVES)
EM_good <- filter(pattern_extension_methods_ds, def_group_id==from_group_id | def_group_id==to_group_id)

overview_table(
  r("EM class over def", EM_class_over_def),
  r("EM class over def pct", ratio(EM_class_over_def, pattern_extension_methods_ds)),
  r("EM conditional", EM_conditional),
  r("EM conditional pct", ratio(EM_conditional, pattern_extension_methods_ds)),
  r("EM good", EM_good),
  r("EM good pct", ratio(EM_good, pattern_extension_methods_ds)),
  r("EM java", EM_from_to_java),
  r("EM java pct", ratio(EM_from_to_java, pattern_extension_methods_ds)),
  r("EM java io",      is_from_to(EM_from_to_java, JAVA_IO)),
  r("EM java date",    is_from_to(EM_from_to_java, JAVA_DATE)),
  r("EM java lang",    is_from_to(EM_from_to_java, c("^java/lang/"))),
  r("EM java android", is_from_to(EM_from_to_java, c("^android/"))),
  r("EM java libraries", count(gather(EM_from_to_java, "key", "value", from_group_id, to_group_id), value)),
  r("EM from primitive", EM_from_primitive),
  r("EM from primitive to io", filter(EM_from_primitive, str_detect_multiple(to, JAVA_IO))),
  r("EM from primitive to date", filter(EM_from_primitive, str_detect_multiple(to, JAVA_DATE))),
  r("EM from String", filter(count(EM_from_primitive, from), from == "java/lang/String#")$n),
  r("EM from Int", filter(count(EM_from_primitive, from), from == "scala/Int#")$n),
  r("EM conditional TC", semi_join(EM_conditional, pattern_type_class_ds, by="declaration_id")),
  r("EM conditional CTX", semi_join(EM_conditional, pattern_context_ds, by="declaration_id"))
)
```


### Type classes (Section 5.3.3)

```{r}
TC_ds_per_project <- count(pattern_type_class_ds, project_id)
TC_cs_per_project <- count(pattern_type_class_cs, project_id)
TC_declaration    <- count(pattern_type_class_cs, def_library, declaration_id)
save_top_n_projects(TC_ds_per_project, TC_cs_per_project, path(output_dir, "TC-top-projects.tex"))
```

### Extension syntax methods (Section 5.3.4)

```{r}
ESM_ds_per_project <- count(pattern_extension_methods_ds, project_id)
ESM_cs_per_project <- count(pattern_extension_methods_cs, project_id)
ESM_declaration    <- count(pattern_extension_methods_cs, def_library, declaration_id)
save_top_n_projects(ESM_ds_per_project, ESM_cs_per_project, path(output_dir, "ESM-top-projects.tex"))
```

### Type Proofs (Section 5.3.5)

```{r}
TP_ds_per_project <- count(pattern_type_proofs_ds, project_id)
TP_cs_per_project <- count(pattern_type_proofs_cs, project_id)
TP_declaration    <- count(pattern_type_proofs_cs, def_library, declaration_id)
save_top_n_projects(TP_ds_per_project, TP_cs_per_project, path(output_dir, "TP-top-projects.tex"))
```

### Context (Section 5.3.6)

```{r}
CTX_ds_per_project <- count(pattern_context_ds, project_id)
CTX_cs_per_project <- count(pattern_context_cs, project_id)
CTX_declaration    <- count(pattern_context_cs, def_library, declaration_id)
save_top_n_projects(CTX_ds_per_project, CTX_cs_per_project, path(output_dir, "CTX-top-projects.tex"))
```

```{r}
parameters_local_context %>% count(resolved_type_id)
filter(parameters_local_context, resolved_type_id=="scala/Function1#")

parameters_local_context_jdk <- filter(parameters_local_context, type_group_id=="JDK")
parameters_local_context_primitive <- filter(parameters_local_context, declaration_is_scala_primitive(resolved_type_id))
parameters_local_context_function <- filter(parameters_local_context, startsWith(resolved_type_id, "scala/Function"))

overview_table(
  r("CTX JDK", parameters_local_context_jdk),
  r("CTX JDK types", count(parameters_local_context_jdk, type_id)),
  r("CTX JDK projects", count(parameters_local_context_jdk, project_id)),
  r("CTX primitive", parameters_local_context_primitive),
  r("CTX primitive projects", count(parameters_local_context_primitive, project_id)),
  r("CTX function", parameters_local_context_function),
  r("CTX function projects", count(parameters_local_context_function, project_id)),
)
```
...
### Anti-pattern Conversions (Section 5.3.7)

#### Unrelated

```{r}
UIC_ds_per_project <- count(pattern_unrelated_conversions_ds, project_id)
UIC_cs_per_project <- count(pattern_unrelated_conversions_cs, project_id)
UIC_declaration    <- count(pattern_unrelated_conversions_cs, def_library, declaration_id)
save_top_n_projects(UIC_ds_per_project, UIC_cs_per_project, path(output_dir, "UIC-top-projects.tex"))
```

source or target type. Summary is in Table 15.
There are 41K of unrelated conversions spanning across 1.1K projects. Most of them belong
to the already mentioned slinky projects bringing React apps development to Scala. The rest is
quite closely collocated with its source or target. If we change the query to only regard the same
artifact then it drops too 1.1K conversions in 619 projects. There are some indication that unrelated
conversions might be deprecated in the upcoming revision of the Scala language 12 here show, that these conversions are being defined, but they are usually in the scope of the same
library. 

From the unrelated conversions, 1.6K from 550 projects involves Scala primitive. Only a
few convert just between primitive types and these are defined mainly in small applications. The
rest spans both applications and libraries.

```{r}
overview_table(
  r("UIC defined", sum(UIC_ds_per_project$n)),
  r("UIC defined in slinky", filter(UIC_ds_per_project, project_id=="shadaj--slinky")$n),
  r("UIC defining projects ", UIC_ds_per_project),
  r("UIC defining projects pct", ratio(UIC_ds_per_project, corpus_noncat)),
  r("UIC using projects ", UIC_cs_per_project),
  r("UIC using projects pct", ratio(UIC_cs_per_project, corpus_noncat)),
  r("UIC defined in libs", unrelated_conversions_lib_ds),
  r("UIC defined in libs projects", count(unrelated_conversions_lib_ds, project_id))
)
```
...
```{r}
UIC_involving_primitive <- filter(pattern_unrelated_conversions_ds, declaration_is_scala_primitive(from) | declaration_is_scala_primitive(to))
UIC_involving_primitive_both <- filter(UIC_involving_primitive, declaration_is_scala_primitive(from), declaration_is_scala_primitive(to))

overview_table(
  r("UIC primitive", UIC_involving_primitive),
  r("UIC primitive projects", count(UIC_involving_primitive, project_id)),
  r("UIC from primitive", filter(UIC_involving_primitive, declaration_is_scala_primitive(from))),
  r("UIC to primitive", filter(UIC_involving_primitive, declaration_is_scala_primitive(to))),
  r("UIC both primitive", UIC_involving_primitive_both),
  r("UIC both primitive projects", count(UIC_involving_primitive_both, project_id))
)
```

- Most are string and ints

#### Bi-directional

```{r}
BIC_ds_per_project <- count(pattern_bi_directional_ds, project_id)
BIC_cs_per_project <- count(pattern_bi_directional_cs, project_id)
BIC_declaration    <- count(pattern_bi_directional_cs, def_library, declaration_id)
save_top_n_projects(BIC_ds_per_project, BIC_cs_per_project, path(output_dir, "BIC-top-projects.tex"))
```

```{r}
overview_table(
  r("BIC defined", sum(BIC_ds_per_project$n)),
  r("BIC defining projects", BIC_ds_per_project),
  r("BIC defining projects pct", ratio(BIC_ds_per_project, corpus_noncat)),
  r("BIC using projects", BIC_cs_per_project),
  r("BIC using projects pct", ratio(BIC_cs_per_project, corpus_noncat)),
)
```

- From Java / to Java version of the API (Akka, Spark)
- DSL onstructs
  - using a tuple to represent a cell
  - lifting from / to Options
- converting between dates

```{r}
overview_table(
  r("BIC from or to primitives", filter(pattern_bi_directional_ds, declaration_is_scala_primitive(from))),
  r("BIC from and to primitives", filter(pattern_bi_directional_ds, declaration_is_scala_primitive(from), declaration_is_scala_primitive(to)))
)
```

Projects using WrapAs / DecorateAs

```{r}
BIC_decorate_as <- filter(callsites_all, startsWith(declaration_id, "scala/collection/convert/DecorateAs")) %>% count(project_id)
BIC_wrap_as <- filter(callsites_all, startsWith(declaration_id, "scala/collection/convert/WrapAs")) %>% count(project_id)
BIC_projects_using_both <- filter(corpus_cat, cat!="test") %>% semi_join(semi_join(BIC_wrap_as, BIC_decorate_as, by="project_id"), by="project_id")

overview_table(
  r("BIC projects using decorate as", BIC_decorate_as),
  r("BIC projects using decorate as pct", ratio(BIC_decorate_as, corpus_noncat)),
  r("BIC projects using wrap as", BIC_wrap_as),
  r("BIC projects using wrap as pct", ratio(BIC_wrap_as, corpus_noncat)),
  r("BIC projects using both", BIC_projects_using_both)
)
```
...
implicit_not_found <- match_implicit_not_found(parameters_local)
implicit_not_found_types <- distinct(implicit_not_found, type_library, type_id)
implicit_not_found_projects <- distinct(implicit_not_found_types, type_library)

overview_table(
  r("implicit not found use", implicit_not_found),
  r("implicit not found types", implicit_not_found_types),
  r("implicit not found projects", implicit_not_found_projects)
)
```
...
```{r}
filter(pattern_implicit_conversions_ds, from_language=="JAVA", to_language=="JAVA")
IC_from_java <- 
...
### Implicits and Code Comprehension
...
decl_w_params <- 
  ...
  M <- max(decl_w_params$num_implicit_parameters)
  N <- nrow(decl_w_params)
  BW <- 1
  
  decl_w_params %>% 
    filter(num_implicit_parameters<=10) %>%
    ggplot(aes(num_implicit_parameters)) + 
    geom_histogram(color="#ffffff", fill="#EFA18F", center=0, binwidth=BW) +
    stat_bin(binwidth=BW, center=0, aes(label=scales::percent(..count../N), y=..count..), geom="text", vjust=.5, hjust=1.5, color="black", angle=90) +
    #coord_cartesian(xlim = c(1, M)) + 
    scale_x_continuous(breaks = seq(0,M,BW), labels = seq(0,M,BW)) +
    ...
    labs(
      x="Number of implicit parameters",
      y="Number of declarations (log)"
    ) + 
    coord_cartesian(ylim = c(200,2e5), xlim=c(0, 10))
})
```

```{r}
ggsave(path(output_dir, "hist-impl-parameters.pdf"), width=4.49, height=7.85, unit="in", device=cairo_pdf)
```
...
local({
  decl_w_chain <- 
    callsites_all %>%
    filter(is.na(parent_id), def_kind %in% c("DEF", "CLASS")) %>%
    mutate(all=replace_na(nested_calls_count,0)+replace_na(nested_values_count, 0))

  M <- max(decl_w_chain$all)
  BW <- 1
  N <- nrow(decl_w_chain)
...
decl_w_chain %>% 
  filter(all <= 10) %>%
  ggplot(aes(all)) + 
  geom_histogram(color="#ffffff", fill="#507DD4", center=0, binwidth = BW) +
  stat_bin(binwidth=BW, center=0, aes(label=scales::percent(..count../N), y=..count..), geom="text", vjust=.5, hjust=1.5, color="black", angle=90) +
  scale_x_continuous(breaks = seq(0,M,BW), labels = seq(0,M,BW)) +
  scale_y_log10(labels=scales::comma) +
  theme_minimal() +
  ...
  labs(
    x="Number of all injected arguments including all nested calls",
    y="Number of call sites (log)"
  ) +
  coord_cartesian(ylim = c(1000,2e6), xlim=c(0, 10))
})
```

```{r}
ggsave(path(output_dir, "hist-nested-arguments.pdf"), width=4.61, height=7.85, unit="in", device=cairo_pdf)
```

### Section 5.3

```{r}
inject_arguments <- summarise(filter(callsites_all, is.na(parent_id), !is.na(nested_declarations_length)), x=sum(nested_declarations_length))$x
scala_code_size <- filter(corpus_noncat, project_id=="scala--scala")$scala_code
chars_per_line <- 100

overview_table(
  r("length of injected declarations", inject_arguments),
  r("scala code size", scala_code_size),
  r("length of injected declarations", inject_arguments/scala_code_size/chars_per_line),
  r("number of chars per line", 100)
)
```

```{r}
callsites_all_nested <- 
  callsites_all %>%
  filter(is.na(parent_id), def_kind %in% c("DEF", "CLASS")) %>%
  mutate(all=replace_na(nested_calls_count,0) + replace_na(nested_values_count, 0))

callsites_all_nested_max <- max(callsites_all_nested$all)
overview_table(
  r("cs nesting median", median(summary(callsites_all_nested$all))),
  r("cs nesting max", callsites_all_nested_max),
  # https://github.com/xdotai/typeless/blob/0418ec193efcf6cf640951f70ca8a44c6a23ada8/src/test/scala/crunch.scala#L103
  r("cs nesting max project", filter(callsites_all_nested, all==callsites_all_nested_max)$project_id %>% str_replace("--", "/")),
  r("cs nesting max char length", filter(callsites_all_nested, all==callsites_all_nested_max)$nested_declarations_length),
  r("cs nesting max url", "url")
)

filter(callsites_all_nested, all > 100) %>% select(project_id, all, github_url) %>% arrange(desc(all))
```
