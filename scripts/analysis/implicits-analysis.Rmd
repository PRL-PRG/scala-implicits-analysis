...
#  geom_flat_violin(width=.5, alpha=.7, lwd=.2, trim = T, scale="width", position = position_dodge(.8), draw_quantiles=1) +
  ...
  geom_boxplot(aes(fill=cat), width=.5, alpha=.7, lwd=.2, outlier.size=0, outlier.stroke=0, position=position_dodge(.8), color="black") +
...
### Unrelated Implicits (Section 5.3.7)
...
```{r}
ggsave(path(output_dir, "hist-impl-parameters.pdf"))
```
...
```{r}
ggsave(path(output_dir, "hist-nested-arguments.pdf"))
```
