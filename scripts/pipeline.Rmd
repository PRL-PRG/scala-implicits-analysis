...
params:
  base_dir: "/home/rstudio/work/Research/Projects/scala-corpus/corpora/6-scaladex"
...
```{r}
metadata_status <- merge_project_csvs(project_ids, projects_file(project_ids, METADATA_STATUS), col_types="ii")
compile_status <- merge_project_csvs(project_ids, projects_file(project_ids, COMPILE_STATUS), col_types="ii")
semanticdb_status <- merge_project_csvs(project_ids, projects_file(project_ids, SEMANTICDB_STATUS), col_types="ii")
implicits_status <- merge_project_csvs(project_ids, projects_file(project_ids, IMPLICITS_STATUS), col_types="ii")
# project_id,failure,declarations,callsites,failures
implicits_stats <- merge_project_csvs(project_ids, projects_file(project_ids, IMPLICITS_STATS), col_types="cciii")
```
...
```{r}
exceptions <- merge_project_csvs(project_ids, projects_file(project_ids, IMPLICITS_EXCEPTIONS), col_types="ccccc")
```
